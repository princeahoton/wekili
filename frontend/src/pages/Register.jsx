...
function Register() {
  ...
  useEffect(() => {
    ...
    const initGoogle = () => {
      window.google?.accounts.id.initialize({ client_id: GOOGLE_CLIENT_ID, callback: handleGoogleCredential, use_fedcm_for_prompt: false });
    };
  ...
  const handleGoogleClick = () => {
    if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID.startsWith('REMPLACE')) { setError('Connexion Google non configurée'); return; }
    window.google?.accounts.id.prompt() || setError('SDK Google non chargé. Rechargez la page.');
  };
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-[55%] flex flex-col justify-center px-6 sm:px-12 lg:px-14 py-10 bg-white overflow-y-auto">
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          {/* Google */}
          <button onClick={handleGoogleClick} disabled={loading} className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl py-3.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all mb-3 disabled:opacity-50">
