  ...
  useEffect(() => {
    ...
    getProfile().then((res) => {
      if (res?.profile) {
        ...
        if (!Array.isArray(p.pays_cibles)) p.pays_cibles = [];
        setForm((prev) => ({ ...prev, ...p }));
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className={`fixed left-0 top-0 h-full w-64 md:w-56 bg-white border-r border-gray-100 flex flex-col z-40 transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="px-5 py-5 border-b border-gray-100 flex items-center justify-between">
          <a href="/dashboard" onClick={(e) => { e.preventDefault(); navigate('/dashboard', { replace: window.location.pathname === '/dashboard' }); }} className="flex items-center gap-2">
        ...
        <div className="p-4 md:p-8 pb-24 md:pb-8 flex-1">
          <div className="max-w-2xl mx-auto">
            ...
            <div className="flex items-center justify-between mt-6">
              <button
                onClick={() => etape > 1 ? setEtape(etape - 1) : navigate(-1)}
